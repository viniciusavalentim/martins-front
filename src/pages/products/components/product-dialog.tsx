...
export function ProductDialog({ product }: ProductDialogProps) {
  ...
  const { mutateAsync: createProductFn, isPending: isPendingCreate } =
    useMutation({
  ...
  const { mutateAsync: updateProductFn, isPending: isPendingUpdate } =
    useMutation({
  ...
  const calculatePricing = () => {
    ...
    // NOVO: Pegar o valor de venda do formData
    const sellingPriceInput =
      Number.parseFloat(formData.sellingPrice.replace(",", ".")) || 0;
    // const sellingPrice = sellingPriceInput > ctp ? sellingPriceInput : ctp; // Garante que o preço de venda não é menor que o CTP (opcional)
    const sellingPrice = sellingPriceInput; // Garante que o preço de venda não é menor que o CTP (opcional)

    let profitMarginPorcent = 0;
    let unitProfit = 0;

    if (sellingPrice > ctp && ctp > 0) {
      unitProfit = sellingPrice - ctp;
      profitMarginPorcent = (unitProfit / ctp) * 100;
    }
  ...
  const updateRecipeItemId = (index: number, value: string) => {
    ...
    newRecipe[index] = {
      ...newRecipe[index],
      material: Materials?.find((x) => x.id === value),
      quantityUsed: 0,
      id: "",
      materialId: value,
    };
  ...
  const addAdditionalCost = () => {
    setAdditionalCosts([
      ...(additionalCosts ?? []),
      { id: "", type: "FIXED_VALUE", value: 0, description: "", productId: "" },
    ]);
  };

  const updateAdditionalCost = (
    index: number,
    field: keyof ProductAdditionalCost,
    value: string | number
  ) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { ctp, sellingPrice, profitMarginPorcent } = calculatePricing();
    ...
    const productData = {
      ...
      additionalCosts,
      ctp,
      profitMargin: profitMarginPorcent,
      sellingPrice,
    };
        ...
        <DialogHeader>
          ...
          <DialogDescription>
            {product
              ...
              : "Crie um produto com receita e precificação"}
          </DialogDescription>
        </DialogHeader>
              ...
              <CardContent className="space-y-3">
                {productMaterial.length === 0 ? (
                  ...
                  productMaterial.map((item, index) => {
                    const insumo = item.material;
                    const itemCost = insumo
                      ? insumo.unitCost * item.quantityUsed
                      : 0;
                    return (
                      <div key={index} className="flex gap-2 items-end">
                        <div className="flex-1 grid gap-2">
                          ...
                          <Select
                            ...
                            onValueChange={(value) =>
                              updateRecipeItemId(index, value)
                            }
                          >
                    ...
                    <div key={cost.id} className="flex gap-2 items-end">
                      ...
                      <div className="w-58 grid gap-2">
                        ...
                        <Select
                          value={cost.type}
                          onValueChange={(value) =>
                            updateAdditionalCost(index, "type", value)
                          }
                        >
                          <SelectTrigger className="min-w-[180px]">
                            <SelectValue />
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-muted-foreground">
                    <span>
                      Margem de Lucro ({profitMarginPorcent.toFixed(1)}%):
                    </span>
