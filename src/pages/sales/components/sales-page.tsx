...
export function SalesPage() {
  ...
  const [additionalCosts, setAdditionalCosts] = useState<
    Omit<OrderAdditionalCost, "id" | "orderId">[]
  >([]);
  ...
  const [, setShowNewCustomer] = useState(false);
  ...
  useEffect(() => {
    ...
    if (isLinked) {
      const linkedItem = items.find(
        (item) => item.productId === currentCost.orderItemId
      );
  ...
  const handleAddItem = () => {
    ...
    const expectedProfit =
      (product.sellingPrice - product.totalCost) * quantity;
  ...
  const handleAddCost = () => {
    ...
    if (
      ...
      toast.error(
        "Não é possível adicionar o custo. Verifique a disponibilidade do produto vinculado."
      );
      return;
    }
    ...
    const newCost: Omit<OrderAdditionalCost, "id" | "orderId"> = {
      ...
      amount,
      orderItemId:
        currentCost.orderItemId === "none"
          ? undefined
          : currentCost.orderItemId,
      category: currentCost.category,
      quantity:
        currentCost.orderItemId !== "none"
          ? Number.parseFloat(currentCost.quantity)
          : undefined,
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createSaleFn({
        ...
        nameCustomer: newCustomer.name ?? undefined,
        emailCustomer: newCustomer.email ?? undefined,
        phoneCustomer: newCustomer.phone ?? undefined,
        orderStatus: (() => {
          switch (status) {
            case "PENDING":
              return 1;
                      ...
                      <Alert variant="destructive">
                        ...
                        <AlertDescription>
                          <div className="font-medium mb-1">
                            Não é possível vender este produto:
                          </div>
                          ...
                          {availabilityStatus.missingInsumos && (
                            <div className="text-xs mt-1">
                              Insumos faltantes:{" "}
                              {availabilityStatus.missingInsumos.join(", ")}
                            </div>
                          )}
                        </AlertDescription>
                      </Alert>
                    ...
                    {availabilityStatus.status === "can_produce" && (
                      <Alert>
                        ...
                        <AlertDescription>
                          ...
                          <div className="text-sm">
                            Não há estoque do produto, mas há insumos
                            suficientes. O produto será produzido
                            automaticamente ao finalizar o pedido (se status não
                            for PENDENTE).
                          </div>
                        </AlertDescription>
                      </Alert>
                      ...
                      >
                        <div className="flex-1 space-y-1">
                          ...
                          <div className="text-xs text-green-600">
                            Lucro: {formatCurrency(item.realProfit)}
                          </div>
                        </div>
                        <Button
                          type="button"
                        ...
                        <Select
                          ...
                          onValueChange={(value) =>
                            setCurrentCost({
                              ...currentCost,
                              category:
                                value as OrderAdditionalCost["category"],
                            })
                          }
                        >
                          ...
                          <Alert>
                            ...
                            <AlertDescription>
                              ...
                              <div className="text-sm">
                                ...
                                acionada ao finalizar o pedido.
                              </div>
                            </AlertDescription>
                          </Alert>
                        )}
                      ...
                      <div className="space-y-2">
                        {additionalCosts.map((cost, index) => {
                          const linkedItem = cost.orderItemId
                            ? items.find(
                                (i) => i.productId === cost.orderItemId
                              )
                            : null;
                          return (
                            <div
